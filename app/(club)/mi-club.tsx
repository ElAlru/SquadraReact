...
export default function MiClub() {
  ...
  if (!data) {
    return (
      <View style={[styles.emptyWrap, { backgroundColor: c.fondo }]}>
        <Text style={[styles.emptySub, { color: c.subtexto }]}>No se pudo cargar la información</Text>
      </View>
    )
  }
    ...
    <View style={{ flex: 1, backgroundColor: c.fondo }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
            {teams.map((t: any) => {
              ...
              return (
                ...
                >
                  <Text style={{ color: selected ? '#fff' : c.texto, fontWeight: selected ? 'bold' : '500', fontSize: 13 }}>
                    {t.category} {t.suffix}
                  </Text>
                </TouchableOpacity>
              )
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.chipsRow}>
          {[
            ...
            <View key={label} style={[styles.chip, { backgroundColor: c.input, borderColor: c.bordeInput }]}>
        ...
        {data.staff?.length > 0 ? (
          <View style={styles.staffList}>
            {data.staff.map((m: any) => (
              <View key={m.id} style={[styles.staffCard, { backgroundColor: c.input, borderColor: c.bordeInput }]}>
                <View style={[styles.staffAvatar, { backgroundColor: `${c.boton}18`, borderColor: `${c.boton}35` }]}>
                  <Text style={[styles.staffAvatarText, { color: c.boton }]}>{m.firstName?.charAt(0)}</Text>
                </View>
                <View style={styles.staffInfo}>
                  <Text style={[styles.staffNombre, { color: c.texto }]}>{m.firstName} {m.lastName}</Text>
                  {m.phone && <Text style={[styles.staffPhone, { color: c.subtexto }]}>📞 {m.phone}</Text>}
                  {m.staffRole && <Text style={[styles.staffPhone, { color: c.subtexto }]}>{m.staffRole}</Text>}
                </View>
              </View>
            ))}
          </View>
        ...
        {/* ─── PLANTILLA ──────────────────────────────────────────────────── */}
        <Text style={[styles.sectionTitle, { color: c.texto, marginTop: 24 }]}>⚽ Plantilla</Text>
              ...
              return (
                ...
                >
                  <View style={[styles.jugadorAvatar, { backgroundColor: `${posColor}18`, borderColor: `${posColor}35` }]}>
                    <Text style={[styles.jugadorAvatarText, { color: posColor }]}>{j.firstName?.charAt(0)}</Text>
                  </View>
                  <View style={styles.jugadorInfo}>
                    <Text style={[styles.jugadorNombre, { color: c.texto }]}>{j.firstName} {j.lastName}</Text>
                    <View style={styles.jugadorMeta}>
                      {j.birthDate && <Text style={[styles.jugadorMetaText, { color: c.subtexto }]}>🎂 {j.birthDate}</Text>}
                      {j.kitSize  && <Text style={[styles.jugadorMetaText, { color: c.subtexto }]}>👕 {j.kitSize}</Text>}
                    </View>
                    {j.docNumber && (
                  ...
                  <View style={styles.jugadorDerecha}>
                    <View style={[styles.dorsalBadge, { backgroundColor: `${posColor}18`, borderColor: `${posColor}35` }]}>
                      <Text style={[styles.dorsalText, { color: posColor }]}>#{j.jerseyNumber || '?'}</Text>
                    </View>
                    <Text style={[styles.posicionText, { color: c.subtexto }]}>{POSICION_LABEL[j.position] || 'Jugador'}</Text>
                    <Text style={[styles.statsHint, { color: c.boton }]}>📊 Stats</Text>
                  </View>
                </TouchableOpacity>
              )
            })}
      ...
      <Modal visible={statsModal} transparent animationType="slide" onRequestClose={() => setStatsModal(false)}>
        <Pressable style={styles.overlay} onPress={() => setStatsModal(false)}>
          <Pressable style={[styles.statsCard, { backgroundColor: c.fondo, borderColor: c.bordeInput }]} onPress={() => {}}>
            ...
            <View style={styles.statsHeader}>
              <View>
                ...
                <Text style={[styles.statsPos, { color: c.subtexto }]}>
                  {POSICION_LABEL[selectedPlayer?.position] || 'Jugador'} · #{selectedPlayer?.jerseyNumber || '?'}
                </Text>
              </View>
            ...
            {loadingStats ? (
              <ActivityIndicator color={c.boton} style={{ marginTop: 30, marginBottom: 30 }} />
            ) : playerStats ? (
              <>
                ...
                <View style={styles.statsGrid}>
                  {[
                    ...
                    { label: '🅰️ Asistencias',    value: playerStats.totalAsistencias },
                    { label: '🏆 Partidos gan.',   value: playerStats.partidosGanados },
                    { label: '🎮 Partidos',         value: playerStats.totalPartidos },
                    { label: '🟨 T. Amarillas',    value: playerStats.totalTarjetasAmarillas },
                    { label: '🟥 T. Rojas',        value: playerStats.totalTarjetasRojas },
                  ].map(stat => (
                    <View key={stat.label} style={[styles.statBox, { backgroundColor: c.input }]}>
